...
float xBase = (4.0 * a / p) * abs(mod(uv.x - p * 0.25, p) - p * 0.5) - a;
float yBase = (4.0 * a / p) * abs(mod(uv.y - p * 0.25, p) - p * 0.5) - a;

a = 100.0;
p = 1000.0;
float xDetail = (4.0 * a / p) * abs(mod(uv.x - p * 0.25, p) - p * 0.5) - a;
float yDetail = (4.0 * a / p) * abs(mod(uv.y - p * 0.25, p) - p * 0.5) - a;

return example(uv) + min(xBase,yBase) + min(xDetail + 100.0,yDetail- 100.0);
}
/*** USER DEFINED FUNCTION END ***/

void main(){
 FragColor = example_terrain();
}